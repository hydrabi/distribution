...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(width*(1-TAB_BAR_LINE_WIDTH_PERCENT)/2, NAV_TAB_BAR_HEIGHT - 3.0f, width*TAB_BAR_LINE_WIDTH_PERCENT, 3.0f)];
...
//        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
        CustomArrowButton *button = [[CustomArrowButton alloc] initWithFrame:CGRectMake(buttonX, DOT_COORDINATE, [widths[index] floatValue], NAV_TAB_BAR_HEIGHT) barButtonType:index];
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self configerTitleColorWithSelectedIndex:0];
    return buttonX;
}

-(void)configerTitleColorWithSelectedIndex:(NSInteger)index{
    [_items enumerateObjectsUsingBlock:^(CustomArrowButton *button,NSUInteger idx,BOOL *stop){
        [button modifyTextColorWithSelectedOrNot:idx == index?YES:NO];
    }];
}
...
//平均宽度
-(NSArray*)getDivideEquallyScreenWidthWithTitles:(NSArray*)titles{
    CGFloat screenWidth = [UIScreen mainScreen].bounds.size.width;
    CGFloat width = titles.count>0?screenWidth/titles.count:screenWidth;
    NSMutableArray *widths = @[].mutableCopy;
    for(NSInteger i =0 ;i<titles.count;i++){
        NSNumber *widthNumber = [NSNumber numberWithFloat:width];
        [widths addObject:widthNumber];
    }
    return widths;
}
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x + [_itemsWidth[currentItemIndex] floatValue]*(1-TAB_BAR_LINE_WIDTH_PERCENT)/2, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue]*TAB_BAR_LINE_WIDTH_PERCENT, _line.frame.size.height);
    }];
    
    [self configerTitleColorWithSelectedIndex:_currentItemIndex];
}
...
- (void)updateData
{
    ...
    _itemsWidth = [self getDivideEquallyScreenWidthWithTitles:_itemTitles];
...
#pragma mark -
- (void)itemPressedWithIndex:(NSInteger)index button:(UIButton *)button
{
